...
**Architecture:** 只新增 `.github/workflows/release.yml`。workflow 分成 `ensure-release-readiness` 與 `release` 兩個 jobs：前者處理版本與外部狀態檢查，後者處理 branch/head 驗證、建置驗證、local release state、npm publish、git push 與 GitHub Release；release 順序以「先更新 package.json version，再 `npm pack --dry-run`，再建立 release body」為準。
...
```yaml
      ...
      - name: Bump package version without lifecycle hooks
        env:
...
- [ ] **Step 3: 加入 release body、local commit 與 local tag**
...
```yaml
      - name: Publish to npm
        ...
        run: |
          ...
          if [ -f .npmrc ] && grep -Eq '(^|[[:space:]])(_authToken|authToken)' .npmrc; then
            echo "Refusing to publish with auth token settings in repo-level .npmrc"
            exit 1
          fi
          NPM_CONFIG_USERCONFIG="$(mktemp)"
          trap 'rm -f "$NPM_CONFIG_USERCONFIG"' EXIT
          printf 'registry=https://registry.npmjs.org/\n' > "$NPM_CONFIG_USERCONFIG"
          npm_config_userconfig="$NPM_CONFIG_USERCONFIG" npm publish --provenance --access public --registry=https://registry.npmjs.org/
...
- release body 與 local tag 皆建立於 package.json version 更新與 `npm pack --dry-run` 之後
